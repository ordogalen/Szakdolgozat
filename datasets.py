...
def extract_files(folder_path):
    ...
    train_nums = len(all_lang_folders) - int(len(all_lang_folders) * 0.15) - int(len(all_lang_folders) * 0.05)
    ...
    for i in range(train_nums, train_nums + int(len(all_lang_folders) * 0.10)):
        speaker_id = all_lang_folders[i].split('\\')[1]
        all_files = sorted(glob.glob(all_lang_folders[i]+'\*'))
        for audio_filepath in all_files:
            to_write = audio_filepath + ' ' + str(speaker_id)
    ...
    for i in range(train_nums + int(len(all_lang_folders) * 0.05), len(all_lang_folders)):
        speaker_id = all_lang_folders[i].split('\\')[1]
        all_files = sorted(glob.glob(all_lang_folders[i]+'\*'))
        for audio_filepath in all_files:
            to_write = audio_filepath + ' ' + str(speaker_id)
